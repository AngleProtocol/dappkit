...
export const createVariableScale = (
	name: string,
	size: number,
	...
	[
		noneValue,
		...new Array(size).fill(0).map((_, i) => `var(--${name}-${i + 1})`),
	].reduce((res, variable, i) => Object.assign(res, { [i]: variable }), {});
...
export const radixColors = scales.reduce(
	(palette, scale) =>
		Object.assign(palette, { [scale]: aggregateColors(scale) }),
